...
applyPrim rc p args =
 case (p,args) of
   ...
   (InplaceVUpdateP _,[VList ls, VInt i, v]) -> do
       ...
       pure ls'
   (VSliceP _,[VInt from, VInt len, VList ls]) -> pure $ VList (L.take len (L.drop from ls))
   (VConcatP _, [VList ls]) -> do
       let concatd = foldl (\acc v ->
                           ...
                           ls
       pure $ VList concatd
   (VMergeP _,[VList ls1,VList ls2]) -> pure $ VList (ls1 ++ ls2)
   (GetNumProcessors, []) -> pure $ VInt 1
   -- Don't sort for now.
   (VSortP _, [ls, _fn]) -> pure ls
   (InplaceVSortP _, [ls, _fn]) -> pure ls
   (PrintInt, [VInt n]) -> do
       tell $ string8 (show n)
       pure $ VProd []
